...
def main():
  ...
  if a.demo:
    ...
    daily=['python3 scripts/provider_dispatch.py --mode fetch-jobs > {out}/backup-jobs.json', 'python3 scripts/backup_jobs.py --mode audit --provider all > {out}/backup-jobs-audit.json']; weekly=['python3 scripts/demo_generate.py --output {out}', 'python3 scripts/generate_report.py --input {out} --output {out}/backup-report.html']
