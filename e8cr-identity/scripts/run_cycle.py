...
def main():
  ...
  if a.demo:
    ...
    daily=['python3 scripts/entra_mfa.py --action coverage > {out}/mfa-audit.json', 'python3 scripts/entra_roles.py --action list > {out}/role-audit.json', 'python3 scripts/entra_ca.py --action audit > {out}/ca-audit.json']
    weekly=['python3 scripts/generate_report.py --input {out} --output {out}/identity-report.html']
    env=dict(os.environ)
    if since:
